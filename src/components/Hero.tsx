        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-16 lg:gap-24 items-center">
          <div className="hero-animate">
            ...
            <p className="text-lg md:text-xl text-mist mt-8 max-w-md leading-relaxed">
              The AI calorie tracker that works in seconds, not minutes.
              Snap a photo, scan a barcode, or read a label.
            </p>

            <div className="flex flex-col sm:flex-row gap-3 mt-10">
              <a href="#" className="inline-flex items-center gap-3 bg-ink text-white px-5 py-3 rounded-xl hover:bg-ink/85 transition-colors">
          ...
          <div className="hero-animate-delay flex justify-center lg:justify-end">
            <div className="w-[260px] h-[540px] bg-ink rounded-[40px] p-[5px] shadow-2xl shadow-ink/20">
              <div className="w-full h-full bg-white rounded-[36px] overflow-hidden">
                <div className="flex justify-center pt-3">
                  <div className="w-[80px] h-[26px] bg-ink rounded-full" />
                </div>

                <div className="px-5 pt-4 space-y-4">
                  <div className="text-center">
                    <p className="text-[10px] font-semibold text-mist">Today&apos;s Progress</p>
                    <p className="text-2xl font-black text-ink">
                      1,420 <span className="text-sm font-semibold text-mist">cal</span>
                    </p>
                  </div>
                    ...
                    <div className="relative w-[110px] h-[110px]">
                      <svg viewBox="0 0 120 120" className="w-full h-full -rotate-90">
                        ...
                        <circle cx="60" cy="60" r="50" fill="none" stroke="#F97316" strokeWidth="10"
                          strokeDasharray="314.16" strokeDashoffset="87.96" strokeLinecap="round" />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-lg font-black text-ink">72%</span>
                        <span className="text-[8px] font-semibold text-mist">of goal</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {[
                      { label: 'Protein', pct: 75, color: 'bg-grove' },
                      { label: 'Carbs', pct: 60, color: 'bg-[#3B82F6]' },
                      { label: 'Fat', pct: 45, color: 'bg-[#8B5CF6]' },
                    ].map((m) => (
                      <div key={m.label} className="flex items-center gap-2">
                        <span className="text-[9px] font-semibold text-mist w-10">{m.label}</span>
                        <div className="flex-1 bg-mint rounded-full h-[5px]">
                          <div className={`${m.color} h-full rounded-full`} style={{ width: `${m.pct}%` }} />
                        </div>
                        <span className="text-[9px] font-bold text-ink w-6 text-right">{m.pct}%</span>
                      </div>
                    ))}
                  </div>

                  <div className="bg-sage rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-md bg-grove/10 flex items-center justify-center text-xs">🍕</div>
                        <div>
                          <p className="text-[10px] font-bold text-ink">Lunch</p>
                          <p className="text-[9px] text-mist">645 cal</p>
                        </div>
                      </div>
                      <div className="text-grove text-[10px] font-bold">7.2</div>
                    </div>
                  </div>

                  <div className="bg-sage rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-semibold text-ink">Health Score</span>
                      <span className="text-base font-black text-grove">8.1</span>
                    </div>
                    <div className="mt-1.5 bg-border-light rounded-full h-[4px]">
                      <div className="bg-grove h-full rounded-full" style={{ width: '81%' }} />
                    </div>
                  </div>
                </div>
              </div>
